...
def validate():
    targets = [ROOT/'index.html', ROOT/'presentation/index.html',
               EXPORT_ROOT/'riskops-control-tower.html', EXPORT_ROOT/'riskops-control-tower-case-study.html']
    references = 0
    for path in targets:
        content = path.read_text(encoding='utf-8')
        assert not re.search(r'__[A-Z_]+__|/\* EMBED_', content), f"Unresolved build token: {path}"
        assert 'risk_summary.json' not in content, f"Old artifact reference: {path}"
        ...
        for reference in page.references:
            ...
            references += 1
        with tempfile.TemporaryDirectory(prefix='riskops-js-check-') as folder:
    ...
    for run in data['dates']:
        ...
        assert run['metrics']['instrument_count'] == 17
    ...
    with zipfile.ZipFile(ROOT/'source.zip') as bundle:
        ...
        for required in ['README.md','riskops/engine.py','scripts/riskops.py','scripts/build_portfolio.py',
